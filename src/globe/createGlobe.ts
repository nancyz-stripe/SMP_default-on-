...
export type GlobeOptions = {
   ...
   *  flat one. */
  dotDarken?: number
  /** Swaps in the coverage-wave shader and returns its setters. */
  coverage?: boolean
...
export type GlobeHandle = {
  /** Coverage variant only: which markets are lit, and where the wave is. */
  setCoverage: (state: { home: number; intl: number; wave: number; waveActive: number }) => void
  dispose: () => void
}
...
export function createGlobe(container: HTMLElement, options: GlobeOptions = {}): GlobeHandle {
  ...
  mapImg.onload = () => {
    ...
    for (let i = 0; i < DOT_COUNT; i++) {
      ...
      if (coverage) {
        ...
        // Great-circle distance from the anchor, normalised to 0–1 so the wave
        // can sweep the whole globe with u_wave running over that range.
        const [x, y, z] = unitVec(latDeg, lngDeg)
        const dot = x * anchorVec[0] + y * anchorVec[1] + z * anchorVec[2]
        dists.push(Math.acos(Math.max(-1, Math.min(1, dot))) / Math.PI)
      }
    }
    ...
    if (coverage) {
      geo.setAttribute('a_region', new THREE.Float32BufferAttribute(regions, 1))
      geo.setAttribute('a_dist', new THREE.Float32BufferAttribute(dists, 1))
    }
  ...
  function createArc() {
    ...
    const cityA = MAJOR_CITIES[Math.floor(Math.random() * MAJOR_CITIES.length)]
    let cityB = MAJOR_CITIES[Math.floor(Math.random() * MAJOR_CITIES.length)]
    while (cityB === cityA) cityB = MAJOR_CITIES[Math.floor(Math.random() * MAJOR_CITIES.length)]
  ...
  return {
    setCoverage({ home, intl, wave, waveActive }) {
      if (!coverage) return
      dotUniforms.u_homeLit.value = home
      dotUniforms.u_intlLit.value = intl
      dotUniforms.u_wave.value = wave
      dotUniforms.u_waveActive.value = waveActive
    },
    dispose() {
      disposed = true
